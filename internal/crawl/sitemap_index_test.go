...
func TestHarvestSitemapIndex(t *testing.T) {
	...
	_, errs := sitemap.SetStorageDestination(tmpStore).Harvest(context.Background(), mockedClient, 10, "test", false, false)
	require.NoError(t, errs)

	_, errs = sitemap.SetStorageDestination(container.ClientWrapper).Harvest(context.Background(), mockedClient, 1, "test", false, false)
	require.NoError(t, errs)
